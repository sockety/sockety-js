  ...
  read: () => (operation) => operation
    ...
    .entry(($) => `
      if (${$.hasBytes(4)}) {
        ...
        ${$.local('temp')} = ${$.bufferAt(0)} | ${$.bufferAt(1, 8)} | ${$.bufferAt(2, 16)};
        ${$.moveOffset(3)}
        ${$.escape('left_1')}
      } else if (${$.hasBytes(2)}) {
    ...
    .snippet('left_3', ($) => `
      if (${$.hasBytes(3)}) {
        ...
        ${$.local('temp')} |= ${$.bufferAt(0, 8)} | ${$.bufferAt(1, 16)};
        ${$.moveOffset(2)}
        ${$.escape('left_1')}
      } else if (${$.hasBytes(1)}) {
    ...
    .snippet('left_2', ($) => `
      if (${$.hasBytes(2)}) {
        ${$.set(`(${$.local('temp')} | ${$.bufferAt(0, 16)}) + (${$.bufferAt(1, 24)})`)}
        ${$.moveOffset(2)}
        ${$.continue()}
      } else if (${$.hasBytes(1)}) {
        ${$.local('temp')} |= ${$.bufferAt(0, 16)};
        ${$.moveOffset(1)}
        ${$.escape('left_1')}
      }
      ${$.escape()}
